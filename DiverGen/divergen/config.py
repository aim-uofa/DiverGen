...
def add_divergen_config(cfg):
    ...
    _C.INPUT.MEAN_STD2_PATH = 'datasets/metadata/area_mean_std2.json'
    ...
    _C.INPUT.INST_POOL_FREQ = ['r', 'c', 'f']
    _C.INPUT.INST_POOL_FILTER_VAL = False
    _C.INPUT.INST_POOL_ORDER_SEED = None
    _C.INPUT.INST_POOL_FIX_ORDER_SEED = False
    ...
    _C.MODEL.USE_DIVERGEN_BOX_LOSS=True ## for abalation
    _C.MODEL.USE_DIVERGEN_MASK_LOSS=True
    ...
    _C.INPUT.RANDOM_SCALE = False
    _C.INPUT.RANDOM_SCALE_MIN = 0.1
    _C.INPUT.RANDOM_SCALE_MAX = 2.0
    _C.INPUT.RANDOM_SCALE_MIN_SIZE = 5
    _C.INPUT.USE_IMGNET = False

    # vit
    _C.MODEL.VIT = CN()
    _C.MODEL.VIT.NAME = "ViT-Base"
    _C.MODEL.VIT.OUT_FEATURES = ["res3", "res4", "res5"]
    _C.MODEL.VIT.USE_CHECKPOINT = False
    _C.MODEL.ENABLE_STATIC_GRAPH = False
