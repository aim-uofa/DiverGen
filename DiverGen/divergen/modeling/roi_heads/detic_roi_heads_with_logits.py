...
class DeticCascadeROIHeadsWithLogits(CascadeROIHeads):
    @configurable
    def __init__(
        self,
        *,
        ...
        **kwargs,
        ...
        self.divergen_mask_loss=divergen_mask_loss
        # TODO
    ...
    def from_config(cls, cfg, input_shape):
        ...
        ret.update({
            ...
            'divergen_mask_loss': cfg.MODEL.USE_DIVERGEN_MASK_LOSS
        })
        return ret
    ...
    def _forward_mask(self, features: Dict[str, torch.Tensor], instances: List[Instances], sem_seg_gt: torch.Tensor =None):
        ...
        if not self.divergen_mask_loss:
            instances_=[]
    ...
    def _forward_box(self, features, proposals, targets=None, 
        ann_type='box', classifier_info=(None,None,None), batch_filename=None):
        """
        ...
        if self.training:
            ...
            logits = [
                sum(list(logits_per_image)) * (1.0 / self.num_cascade_stages) for logits_per_image in zip(logits_per_stage)
            ]
            ...
            # save logits
            for p, l, s, i, f in zip(pred_instances, logits, scores, inds, batch_filename):
                p.pred_inds = i
                
                pred_logits = {}
                pred_scores = {}
                for ind in torch.unique(i):
                    pred_logits[ind.item()] = l[ind].cpu().numpy()
                    pred_scores[ind.item()] = s[ind].cpu().numpy()
                
                assert len(pred_logits) == len(torch.unique(i))
                assert len(pred_scores) == len(torch.unique(i))
                image_id = f.split('/')[-1].split('.')[0]
                np.save('logits/{}.npy'.format(image_id), pred_logits)

                # save scores
                np.save('scores/{}.npy'.format(image_id), pred_scores)
    ...
    def forward(self, images, features, proposals, targets=None,
        ...
        if self.training:
            ...
            pred_instances = self._forward_box(
                features, proposals, classifier_info=classifier_info, batch_filename=kwargs['batch_filename'])
